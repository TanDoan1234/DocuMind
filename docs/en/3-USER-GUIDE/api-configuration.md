...
> **Credential System**: DocuMind uses encrypted credentials stored in the database. Each credential connects to a provider and allows you to discover, register, and test models.

---

## Overview

DocuMind manages AI provider access through a **credential-based system**:
...
3. The models are now available throughout DocuMind
...
For OpenAI, Google, Groq, Mistral, DeepSeek, xAI, OpenRouter:
...
For custom OpenAI-compatible servers (vLLM, etc.):
...
|---------|---------------|---------------------------|
| Password | `documind-change-me` | Set `OPEN_NOTEBOOK_PASSWORD` |
