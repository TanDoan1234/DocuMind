...
> **Alternative Registry:** All images are available on both Docker Hub (`lfnovo/open_notebook`) and GitHub Container Registry (`ghcr.io/lfnovo/documind`). Use GHCR if Docker Hub is blocked or you prefer GitHub-native workflows.
...
```bash
curl -o docker-compose.yml https://raw.githubusercontent.com/lfnovo/documind/main/docker-compose.yml
```
...
Open terminal in the `documind` folder:
...
3. Select your provider (e.g., OpenAI, Google)
...
Done! You now have a fully working DocuMind instance.
...
curl -o docker-compose.yml https://raw.githubusercontent.com/lfnovo/documind/main/examples/docker-compose-ollama.yml
...
docker exec documind-local-ollama-1 ollama pull mistral
```
