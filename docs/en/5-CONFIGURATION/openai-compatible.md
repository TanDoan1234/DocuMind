...
Use any server that implements the OpenAI API format with DocuMind. This includes Text Generation WebUI, vLLM, and many others.
...
DocuMind can connect to any server using this format.
...
When DocuMind runs in Docker and your compatible server runs on the host, use the appropriate base URL when adding your credential in **Settings → API Keys**:
...
```bash
docker exec -it documind curl http://host.docker.internal:1234/v1/models
```
...
4. Update model name in DocuMind
```
