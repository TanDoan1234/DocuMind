...
Ollama provides free, local AI models that run on your own hardware. This guide covers everything you need to know about setting up Ollama with DocuMind, including different deployment scenarios and network configurations.
...
When both DocuMind and Ollama run directly on your machine:
...
### Scenario 2: DocuMind in Docker, Ollama on Host

When DocuMind runs in Docker but Ollama runs on your host machine:
...
When both DocuMind and Ollama run in the same Docker Compose stack:
...
This is the most common cause of "Failed to send message" errors. DocuMind requires the **exact model name** as it appears in Ollama.
...
**Step 2: Use the exact name when adding the model in DocuMind**
...
**Note:** Some models use `:latest` as the default tag. If `ollama list` shows `model:latest`, you must use `model:latest` in DocuMind, not just `model`.

**Step 3: Configure in DocuMind**
...
**1. "Ollama unavailable" in DocuMind**
...
```bash
# If DocuMind runs in Docker or on a different machine,
# Ollama must bind to all interfaces, not just localhost
export OLLAMA_HOST=0.0.0.0:11434
ollama serve
```
> **Why this is needed:** By default, Ollama only accepts connections from `localhost` (127.0.0.1). When DocuMind runs in Docker or on a different machine, it can't reach Ollama unless you configure `OLLAMA_HOST=0.0.0.0:11434` to accept external connections.
...
**From inside DocuMind container, test Ollama:**
```bash
# Get into container
docker exec -it documind bash
...
1. **Model name mismatch**: The model name in DocuMind doesn't exactly match `ollama list`
2. **No default model configured**: You haven't set a default chat model in Settings → Models
3. **Model was deleted**: You removed the model from Ollama but didn't update DocuMind's defaults
4. **Model record deleted**: The model was removed from DocuMind but is still set as default
...
# Compare with what's configured in DocuMind
...
# DocuMind configuration (configure via Settings → API Keys instead)
...
This comprehensive guide should help you successfully deploy and optimize Ollama with DocuMind. Start with the Quick Start section and refer to specific scenarios as needed.