...
Deploy DocuMind behind nginx, Caddy, Traefik, or other reverse proxies with custom domains and HTTPS.

---

## Simplified Setup (v1.1+)

Starting with v1.1, DocuMind uses Next.js rewrites to simplify configuration. **You only need to proxy to one port** - Next.js handles internal API routing automatically.
...
> **Note:** This example only shows the documind and nginx services. You also need a `surrealdb` service. See the [full base docker-compose.yml]() for the complete setup.
...
location /api/ {
    proxy_pass http://documind:5055/api/;
    proxy_http_version 1.1;
    proxy_set_header Host $host;
    proxy_set_header X-Real-IP $remote_addr;
    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
...
Accessing DocuMind from a different machine on your network:

**Step 1: Get your server IP**
```bash
# On the server running DocuMind:
hostname -I
# or
ifconfig | grep "inet "
...
1. **Check API_URL is set**:
   ```bash
   docker exec documind env | grep API_URL
   ```
...
2. Check container logs: `docker logs documind`
...
**Cause:** Your reverse proxy has a default timeout (often 30s) that's shorter than DocuMind's operations.
...
**Version Requirement:**
- **DocuMind v1.3.2+** is required for file uploads >10MB
...
**Note:** DocuMind's API includes CORS headers in error responses, but this only works for errors that reach the application. Proxy-level errors (like 413 from nginx) need to be configured at the proxy level.
...
3. **Reverse proxy not forwarding `/api/*` correctly**:
   ...
   location /api/ {
       proxy_pass http://documind:5055/api/;  # Note the trailing slash!
   }
   ```
...
9. **Monitor logs** regularly: `docker logs documind`
...
If you're running DocuMind **version 1.0.x or earlier**, you may need to use the legacy two-port configuration where you explicitly route `/api/*` to port 5055.

**Check your version:**
```bash
docker exec documind cat /app/package.json | grep version
```
