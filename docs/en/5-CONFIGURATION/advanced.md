...
```env
# Protect your DocuMind instance
...
Restrict access to your DocuMind:
...
DocuMind uses multiple services for content extraction:
...
# Create backup
tar -czf "$BACKUP_DIR/documind-$DATE.tar.gz" \
  /path/to/notebook_data \
  /path/to/surreal_data

# Keep only last 7 days
find "$BACKUP_DIR" -name "documind-*.tar.gz" -mtime +7 -delete

echo "Backup complete: documind-$DATE.tar.gz"
```

Add to cron:
```bash
# Daily backup at 2 AM
0 2 * * * /path/to/backup.sh >> /var/log/documind-backup.log 2>&1
```
...
# On source server
docker compose down
tar -czf documind-migration.tar.gz notebook_data/ surreal_data/

# Transfer to new server
scp documind-migration.tar.gz user@newserver:/path/

# On new server
tar -xzf documind-migration.tar.gz
docker compose up -d
```
