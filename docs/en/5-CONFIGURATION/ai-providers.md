...
DocuMind uses a **credential-based system** for managing AI providers:
...
By default, DocuMind uses the LLM provider's embeddings. Embedding models are discovered and registered through the same credential system — when you discover models from a credential, embedding models are included alongside language models.
