...
Get DocuMind running with **100% local AI** using Ollama. No cloud API keys needed, completely private.
...
Create a new folder `documind-local` and add this file:
...
Open terminal in your `documind-local` folder:
...
docker exec documind-local-ollama-1 ollama pull neural-chat

# OR: Even better quality, more VRAM needed
docker exec documind-local-ollama-1 ollama pull llama2
```
...
## Step 5: Access DocuMind (instant)
...
```bash
# Show available GPUs
docker exec documind-local-ollama-1 ollama ps
...
docker exec documind-local-ollama-1 ollama list
...
4. In DocuMind, go to **Settings** → **API Keys**
